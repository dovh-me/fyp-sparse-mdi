...
class Server(server_pb2_grpc.ServerServicer):
    ...
    async def StartInference(self, request: server_pb2.StartInferenceRequest, context):
        ...
        try:
            ...
            print(f"Inference request received! Queuing new inference task with id: {task_id}") 
            ...
            print('From Server shape:', input_tensor.shape)
            ...
            print(f"There was error processing inference task: {task_id}")
            traceback.print_exc()
    ...
    def update_is_ready(self):
       ...
       print(f"is ready {current_nodes_count} == {required_nodes_count}")
...
async def serve():
    ...
    print(f"Coordinator node started, listening on port {port}")
    ...
    try:
        ...
        print(f"Terminating coordinator node due to a fatal error. Please contact support.")
        await server.stop()
        return
